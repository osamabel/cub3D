...
/*   Updated: 2022/08/21 17:18:48 by obelkhad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	vertical_points(t_data *data, float angle)
{
	...
	while (point_in_range(data, (int)((data->player.v_x - i) / SIZE_), \
	(int)(data->player.v_y / SIZE_)))
	{
		if (is_wall(data, (int)((data->player.v_x - i) / SIZE_), \
		(int)(data->player.v_y / SIZE_)))
			break ;
		data->player.v_distance += hypot(v_dx, v_dy);
