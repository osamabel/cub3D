...
/*   Updated: 2022/08/21 17:18:08 by obelkhad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	horizontal_points(t_data *data, float angle)
{
	...
	while (point_in_range(data, (int)(data->player.h_x / SIZE_), \
	(int)((data->player.h_y - i) / SIZE_)))
	{
		if (is_wall(data, (int)(data->player.h_x / SIZE_), \
		(int)((data->player.h_y - i) / SIZE_)))
			break ;
		data->player.h_distance += hypot(h_dx, h_dy);
