...
/*   Updated: 2022/08/03 13:46:29 by obelkhad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	vertical_points(t_data *data)
{
	...
	while (data->player.ray.v_y < data->col * SIZE_ && data->player.ray.v_y > 0 && !check_wall_v_points(data, data->player.ray.v_x, data->player.ray.v_y))
	{
		v_x = data->player.ray.v_x;
		v_y = data->player.ray.v_y;
		if (cos(data->player.rotatedirection) > 0)
			v_dx = SIZE_ * (int)((v_x + 0.001)/SIZE_) + SIZE_;
		else
			v_dx = SIZE_ * (int)((v_x - 0.001)/SIZE_);
